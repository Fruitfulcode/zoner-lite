...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'zoner-lite' ),
					'defaultString' => __( 'Default', 'zoner-lite' ),
					'pick'          => __( 'Select Color', 'zoner-lite' ),
					'current'       => __( 'Current Color', 'zoner-lite' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __( 'Use this file', 'zoner-lite' ),
			'remove_image'    => __( 'Remove Image', 'zoner-lite' ),
			'remove_file'     => __( 'Remove', 'zoner-lite' ),
			'file'            => __( 'File:', 'zoner-lite' ),
			'download'        => __( 'Download', 'zoner-lite' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '<span class="order-up"></span>',
			'down_arrow'      => '<span class="order-down"></span>',
			'check_toggle'    => __( 'Select / Deselect All', 'zoner-lite' ),
		) ) );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' || $hook == 'user-edit.php' || $hook == 'profile.php') {
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'zoner-lite') );
