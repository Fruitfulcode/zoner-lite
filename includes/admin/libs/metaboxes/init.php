...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'zoner' ),
					'defaultString' => __( 'Default', 'zoner' ),
					'pick'          => __( 'Select Color', 'zoner' ),
					'current'       => __( 'Current Color', 'zoner' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'check_toggle'    => __( 'Select / Deselect All', 'zoner' ),
		) ) );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'zoner') );
